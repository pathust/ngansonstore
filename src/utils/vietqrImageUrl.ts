...
export const getVietQRUrl = (
  ...
  const finalBankId = (bankId || '').trim();
  const finalAccountNo = (accountNo || '').trim().replace(/\s+/g, '');
  if (!finalBankId || !finalAccountNo) return '';
  ...
  const cleanAccName = encodeURIComponent((accountName || '').replace(/[^a-zA-Z0-9 ]/g, ' ').trim().toUpperCase());
